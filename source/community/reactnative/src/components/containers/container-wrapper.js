 ...
 export class ContainerWrapper extends React.PureComponent {
        ...
        const padding = this.props.configManager.hostConfig.getEffectiveSpacing(
         ...
         if (
             ...
             if (this.payload.style === Enums.ContainerStyle.Default) {
                this.hasAncestorBackgroundImage(this.payload) ? computedStyles.push({marginHorizontal: -padding}) : this.hasAncestorStyle(this.payload) &&
     ...
     applyContainerStyle(computedStyles) {
         ...
         if (this.payload.style) {
             if (this.payload.style === Enums.ContainerStyle.Default) {
                (this.hasAncestorBackgroundImage(this.payload) ? computedStyles.push({ marginHorizontal: padding, padding: padding}) : this.hasAncestorStyle(this.payload)) && computedStyles.push({ padding: padding })
             } else {
                computedStyles.push({
                    padding: padding,
                });
